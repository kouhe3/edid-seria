...
mod tests {
    ...
    #[test]
    fn all_presets_pass_dtd_validation() {
        for (i, preset) in all_presets().iter().enumerate() {
            assert!(
                validate_dtd(preset).is_ok(),
                "preset #{} ({}x{} @ {:.1}Hz) failed DTD validation: {:?}",
                i,
                preset.h_active,
                preset.v_active,
                preset.v_rate,
    ...
    #[test]
    fn preset_lookup_matches_standard_resolutions_and_tolerances() {
        let fhd = DetailedTiming::compute_blanking(1920, 1080, 59.94).unwrap();
        assert_eq!(fhd.h_active, 1920);
        assert_eq!(fhd.v_active, 1080);

        let vga = DetailedTiming::compute_blanking(640, 480, 60.0).unwrap();
        assert_eq!(vga.pixel_clock_khz, 25175);

        let xga = DetailedTiming::compute_blanking(1024, 768, 60.0).unwrap();
        assert_eq!(xga.pixel_clock_khz, 65000);

        let sxga = DetailedTiming::compute_blanking(1280, 1024, 75.0).unwrap();
        assert_eq!(sxga.pixel_clock_khz, 135000);

        let uxga = DetailedTiming::compute_blanking(1600, 1200, 60.0).unwrap();
        assert_eq!(uxga.pixel_clock_khz, 162000);

        let hdtv_480p = DetailedTiming::compute_hdtv_blanking(720, 480, 60.0).unwrap();
        assert_eq!(hdtv_480p.pixel_clock_khz, 27000);
